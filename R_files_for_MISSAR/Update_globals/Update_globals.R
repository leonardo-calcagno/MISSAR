...
rm(correct_CPI_url,alt_CPI_url,CPI_url,current_month_exists,last_month_exists)
##Update sheet----
df_latest_CPI<-read_excel("latest_CPI.xls",sheet="Ãndices IPC Cobertura Nacional") %>% 
  rename(CPI_index_type=1) %>%  #Rename the first column
  subset(CPI_index_type=="Nivel general") %>% 
  mutate(CPI_region=ifelse(row_number(CPI_index_type)==1, "Nacional",
                           ifelse(row_number(CPI_index_type)==2, "GBA", 
...
for (i in 1:nrow(vector_PNC)){
  ...
  has_num<-rowSums(mapply(grepl,"[0-9]",input)) #Detecta las columnas con al menos un nÃºmero
...
for(i in 1:nrow(list_PUAM_23)){
  ...
  col_period<-colSums(mapply(grepl,"perÃ­odo|periodo",input,ignore.case=TRUE)) #Detects columns with at least one number
